...
def worker(set_percentage, log_text):
    """
    This work function is called by `progress_dialog`. It will run in a
    background thread.
...
async def main():
    await progress_dialog(title="Progress dialog example", text="As an examples, we walk through the filesystem and print all directories",run_callback=worker).run_async()
